...
--
-- Хост: localhost
-- Время создания: Апр 21 2013 г., 19:31
...
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='Компоненты системы';
...
CREATE TABLE IF NOT EXISTS `menus` (
  ...
  `title` varchar(50) NOT NULL COMMENT 'Заголовок меню (отображаемый на странице)',
  `position` char(30) NOT NULL COMMENT 'Имя позиции к которой привязано меню',
  PRIMARY KEY (`id`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='Меню системы' AUTO_INCREMENT=3 ;
...
CREATE TABLE IF NOT EXISTS `menu_items` (
  ...
  `name` char(50) NOT NULL COMMENT 'Имя пункта меню',
  `title` char(50) NOT NULL COMMENT 'Заголовок',
  `link` char(200) NOT NULL COMMENT 'на что ссылается пункт меню',
...
CREATE TABLE IF NOT EXISTS `positions` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='Позиции шаблона' AUTO_INCREMENT=9 ;
